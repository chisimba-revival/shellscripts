...
TMP_PHP="/tmp/chisimba-create-rubric-fixture.php"
HOST_TMP="$(mktemp /tmp/chisimba-create-rubric-fixture.XXXXXX.php)"
...
echo "=== Chisimba Rubric fixture creator ==="
...
foreach ($existing as $rubric) {
    if (($rubric['title'] ?? '') === $title) {
        echo "EXISTS: rubric fixture already present.\n";
...
if ($structured === false
    ...
    fwrite(STDERR, "FAIL: fixture was created but structured verification failed.\n");
    exit(21);
}
...
if grep -q '^FIXTURE_RESULT=CREATED$' "$LOG"; then
    ...
    echo "PASS: fixture already existed; no duplicate was created."
else
