...
TMP_PHP="/tmp/chisimba-create-rubric-fixture-1738.php"
HOST_TMP="$(mktemp /tmp/chisimba-create-rubric-fixture-1738.XXXXXX.php)"
...
echo "=== Chisimba Rubric fixture creator (corrected context ownership) ==="
echo "Started: $(date -Is)"
echo "Container: $CONTAINER"
echo "Application root: $APP_ROOT"
echo "Context: $CONTEXT_CODE"
echo "Fixture title: $FIXTURE_TITLE"
echo "Log: $LOG"
echo
...
foreach ($existing as $rubric) {
    if (($rubric['title'] ?? '') === $title) {
        $structured = $service->getStructuredRubric($rubric['id']);
        if (is_array($structured)
            && count($structured['criteria'] ?? array()) === 2
            && count($structured['performances'] ?? array()) === 3
        ) {
            echo "EXISTS: verified fixture already present.\n";
...
$description = 'Deterministic rubric fixture for Worksheet integration and Rubric service smoke testing.';
$rubricId = $dbTables->insertSingle($contextCode, $title, $description, 2, 3, null);
if (!$rubricId) {
...
if (!$visible) {
    fwrite(STDERR, "FAIL: newly created context rubric is not visible through listRubrics().\n");
    exit(21);
}

echo "PASS: rubric is visible through listRubrics().\n";
...
if ($structured === false
    ...
    fwrite(STDERR, "FAIL: structured verification failed.\n");
    exit(22);
}

echo "PASS: rubric fixture created and verified through rubricservice.\n";
echo "RUBRIC_ID={$rubricId}\n";
echo "FIXTURE_RESULT=CREATED\n";
PHP

echo "[1/3] Copy temporary creator into PHP 8.5 container"
docker cp "$HOST_TMP" "$CONTAINER:$TMP_PHP" >/dev/null
docker exec "$CONTAINER" php -l "$TMP_PHP"
echo

echo "[2/3] Create and verify fixture"
set +e
docker exec \
...
if grep -q '^FIXTURE_RESULT=CREATED$' "$LOG"; then
    echo "PASS: created fixture '$FIXTURE_TITLE'."
elif grep -q '^FIXTURE_RESULT=EXISTS$' "$LOG"; then
    echo "PASS: verified fixture already existed; no duplicate was created."
else
    echo "FAIL: fixture completion marker not found."
    exit 31
fi

echo "Rubric ID: $RUBRIC_ID"
echo "Next smoke test:"
echo "  ./smoke-rubric-service-20260820-1724.sh \"$CONTEXT_CODE\" \"$RUBRIC_ID\""
